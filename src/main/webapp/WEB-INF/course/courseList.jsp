...
<form class="navbar-form navbar-left" role="search" style="width: 1080px;">
     <div class="form-group" style="width: 1080px;">
       <a href="${pageContext.request.contextPath}/course/addCourse.action" class="btn btn-primary navbar-btn  active " >添加课程</a>
    </div>
</form> 
 ...
 <tr>
   ...
   <c:forEach items="${courseList}" varStatus="status" var="course">
     ...
     <th width="10%">${status.count}</th>
 	 ...
 	 <th width="10%"><a href="${pageContext.request.contextPath}/course/editCourse.action?upid=${course.id}"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a></th>
 	 <th width="10%"><a onclick="del('${pageContext.request.contextPath}/course/deleteCourse.action?delid=${course.id}')"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></th>   	   	
    </tr>  
   </c:forEach>
